...
static __inline uint64_t mul_mod_p(uint64_t a, uint64_t b) {
	uint64_t m = 0;
	while(b) {
		if(b&1) {
			uint64_t t = P-a;
...
static __inline uint64_t pow_mod_p(uint64_t a, uint64_t b) {
	if (b==1) {
		return a;
	}
...
static __inline int b64index(uint8_t c) {
...
// defined in lsha1.c
extern int lsha1(lua_State *L);
extern int lhmac_sha1(lua_State *L);
