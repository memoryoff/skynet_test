...
-- 启动函数，cfg必须是个table，一般包含有 
-- {
-- initial = {event = "",state = ""，defer = true}, 必要选项，没有event项则默认用starup替代。state为初始化后的状态， 定义defer为延迟初始化，需要自己调用doevent初始化
-- events = {name = "",from = "",to = ""}
-- final = {}
-- callbacks = {},
--  如果定义了 cfg.terminal 或 cfg.final 表示有终止状态，使用isFinishedState方法判断
-- }
...
-- 如果leaveState_返回flase表示不能转换，返回StateMachine.ASYNC表示挂起，用户需要自己调用event.transition()，或者调用event.cancel()取消,可以给
-- event.args 传递额外的参数
function StateMachine:doEvent(name, ...)
    ...
    event.transition = function()
        ...
        self:enterState_(event)
        self:changeState_(event) -- 调用 onchangestate()
        self:afterEvent_(event) 
        -- 调用 self.callbacks_["onafter" .. event.name] or self.callbacks_["on" .. event.name],
        --self.callbacks_["onafterevent"] or self.callbacks_["onevent"]
        return StateMachine.SUCCEEDED
    end
...
-- 添加事件，from = *表示可以从任何状态转换，from可以为table，to要是没有，则map[from] = from
