...
function CMD.stop()
	skynet.call("mysqlpool","lua","stop")
end
...
-- 按照表的主键删除
function CMD.delete(tbname, where) 
	-- local pk = schema[tbname]["pk"]
	-- local sql = "delete from " .. tbname.. " where " .. pk .. "=" .. "'" .. row[pk] .. "'"
	where = where or ""
	local sql = "delete from " .. tbname .." "..where
	helper.dump(sql)
	return skynet.call("mysqlpool", "lua", "execute", sql)
end

-- 按照表的主键修改数据
function CMD.update(tbname, row,where)
	local setvalues = ""
	for k, v in pairs(row) do
		setvalues = setvalues .. k .. "='" .. v .. "',"
	end
	setvalues = setvalues:sub(1,-2)
	local sql = "update " .. tbname .. " set " .. setvalues .. " "..where
	helper.dump(sql)
	return skynet.call("mysqlpool", "lua", "execute", sql)
end
...
function CMD.load(tbname,where)
	...
	while true do
		if where then
			sql = string.format("select * from %s " .. where .." limit %d, 1000", tbname, offset)
		else
